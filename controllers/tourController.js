...
exports.resizeImages = catchAsync(async function(req, res, next) {
  req.body.name = 'Corbett National Park';
  if (!req.files.images || !req.files.imageCover)
    return next(new AppError('Please upload an image', 404));

  req.body.imageCover = `${req.user.id}-cover-${Date.now()}`;
  ...
  await Promise.all(
    req.files.images.map(async (file, i) => {
      req.body.images[i] = `${req.user.id}-${i}-${Date.now()}`;
